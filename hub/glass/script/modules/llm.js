...
export function initLLM() {
  ...
  fetchJSON(`/hub/glass/data/llm-agents/default.json`).then(card => {
  ...
  fetchJSON(`/hub/glass/data/llm-agents/index.json`).then(files => {
    if (!Array.isArray(files)) return;
    files.filter(f => !f.includes("default")).forEach(f => {
      fetchJSON(`/hub/glass/data/llm-agents/${f}`).then(card => {
        const data = card.spec ? card.data : card;
