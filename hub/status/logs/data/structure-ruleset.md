...
These use `script/` and `style/` directly at app root (not inside `assets/`) because they import from `/hub/glass/` via absolute paths. The `assets/` convention only applies to fully self-contained apps.
...
**Current valid top-level apps** (all live under `hub/`):

```
hub/art/        /hub/art/                      — ECHOES gallery
hub/playground/ /hub/playground/               — Playground hub (index lists all sub-apps)
...
`hub/playground/` is both a hub page (`hub/playground/index.html`) and a container for sub-apps. Sub-apps are Pattern B apps nested one level inside `playground/`. They follow all standard Pattern B rules except their absolute URL base is `/hub/playground/<name>/` rather than `/<name>/`.

The repo root (`/`) serves only the public portfolio cover (`index.html` + `pf/`). Everything inside `hub/` is the real site, gated by Cloudflare Zero Trust.

---

## 5. The `glass/data/` Layer

`hub/glass/data/` is the single data layer for the hub (`/hub/`) and all `glass/`-integrated apps. Rules:

- One JSON per content domain. No duplicate data across files.
- Every JSON in `glass/data/` has a declared owner in `hub/status/logs/data/logs.json → sources_of_truth`.
